...
**Global dashboard** (`Dashboard.tsx`): combines `globalStats`, `globalMonitor`, `projects`, `financeStats`, `financeData`, `slaStats`, `wfmStats`, `requisitionKpis`, `globalDrilldown` — each maps to `GET` handlers in `main.py` (e.g. `/stats/global`, `/finance/data`, …). On Cloud Run there is no global `/api` prefix; the UI may still call `/api/…` if built for VM nginx — `ApiPrefixStripMiddleware` rewrites those paths.
