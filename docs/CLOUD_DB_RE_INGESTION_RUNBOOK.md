...
| **Database URL**      | Set `DATABASE_URL` **before** any script. **Production:** Cloud SQL Postgres (via proxy or VM). **Local:** `docker compose up -d postgres` then `postgresql+psycopg://tgddata:tgddata_dev@127.0.0.1:5432/tgddata`. See [`POSTGRES_MIGRATION.md`](POSTGRES_MIGRATION.md), [`DEPLOYMENT_DOC.md`](../DEPLOYMENT_DOC.md). |
| **SQLite (legacy)**   | `sqlite:///./revenue_generator.db` only if explicitly set (canonical export: `/Users/arjun/Software/tagged_data_sql/revenue_generator.db` for one-time migrate to Cloud SQL).                                                                                                                        |
