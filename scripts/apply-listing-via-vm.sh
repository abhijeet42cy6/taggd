#!/usr/bin/env bash
# Backfill Listing portfolio fields on Cloud SQL via VM IAP + private-IP Auth Proxy.
#
# Usage (repo root):
#   ./scripts/apply-listing-via-vm.sh           # dry-run
#   ./scripts/apply-listing-via-vm.sh --apply   # commit
#
set -euo pipefail
...
PROXY_PORT="${CLOUDSQL_PROXY_PORT:-9473}"
STAMP="$(date +%Y%m%d%H%M%S)"
REMOTE_DIR="listing-backfill-${STAMP}"
...
echo "==> Packaging listing backfill bundle"
tar czf "${TARBALL}" \
  backend/scripts/apply_listing_portfolio_backfill.py \
  backend/db \
  "excel_files_imp/Copy of Listing.xlsx"
...
echo "==> Listing backfill on Cloud SQL ($([ "${APPLY}" = 1 ] && echo APPLY || echo DRY-RUN))"
...
test -f \"\$WORKDIR/backend/scripts/apply_listing_portfolio_backfill.py\"

CONN='${CLOUDSQL_CONNECTION_NAME}'
USER='${CLOUDSQL_USER}'
DB='${CLOUDSQL_DATABASE}'
PORT=${PROXY_PORT}
PROXY=/tmp/cloud-sql-proxy-listing-backfill
...
docker run --rm --network host \
  ...
  bash -c '
    pip install -q sqlalchemy psycopg[binary] openpyxl python-dotenv &&
    python3 backend/scripts/apply_listing_portfolio_backfill.py ${APPLY_FLAG}
  '
